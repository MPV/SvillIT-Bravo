class GroupMembersController < ApplicationController
  ...
  def index
    ...
    #  @group_members = GroupMembers.find(:condition => { :group_id => params[:group_id] } )
    #else
  ...
  def create
    @group_member = GroupMember.new(params[:group_members])

    respond_to do |format|
      if @group_member.save
        flash[:notice] = 'GroupMembers was successfully created.'
  ...
  def update
    ...
    respond_to do |format|
      if @group_member.update_attributes(params[:group_member])
        flash[:notice] = 'Medlemsskapet uppdaterades.'
